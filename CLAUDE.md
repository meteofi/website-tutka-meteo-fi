...
4. **The animation window is exactly 13 frames** (12 five-minute steps = 1 hour). `FramePool.setWindow` throws on any other length. The number lives in exactly one place — `src/constants.js`, exporting `FRAME_COUNT = 13` and `FRAME_STEPS = 12` — and every module imports it; never re-declare it locally or write a bare `13`/`12` for the window. Use `FRAME_COUNT` for counts and array lengths, `FRAME_STEPS` for spans and last indices (`end - resolution * FRAME_STEPS`, `i <= FRAME_STEPS`); picking the wrong one is this codebase's classic off-by-one (see rule 10). One deliberate exception: `src/nowcast.js` keeps its own `NOW_INDEX = 6` / `FUTURE_STEPS = 6`, which are where the observed/forecast boundary sits, not the window length — but they must keep summing to `FRAME_STEPS`.
...
|---|---|
| `src/constants.js` | Shared app-wide constants — today `FRAME_COUNT` / `FRAME_STEPS` (hard rule 4). Zero imports, so node-harness-testable modules can use it |
...
| `src/tools.js` | Measure + point-probe ("pistemittaus") tools; single `activeTool` state machine for all six tools (arms/disarms the draw modules injected via deps) |
...
| `src/sectionLine.js` | Poikkileikkaus line: drag draws a straight A→B rubber-band line (geometryFunction keeps only first+last vertex; endpoint markers reuse the measure T1/T2 colors); same shared-source / per-pane `Draw` / `onStrokeEnd` pattern as rangeCircle; emits `onLineChange` with the EPSG:4326 endpoints (null on disarm / <1 km release) which drives crossSection.js |
| `src/crossSection.js` | Poikkileikkaus controller + bottom panel (probe-chart height-transition pattern): auto-picks the radar for the line (worse-endpoint distance within `coverage_radius_m`, disk-served single variant preferred via the radarSite.js `isLayerAdvertised` gate, header dropdown override), renders the EDR trajectory `Section` as a distance × height dBZ canvas heatmap, follows the clock via `setTime`-routed `setCursor`. Requested datetimes are taken **verbatim** from collection metadata `.extent.temporal.values` — off-grid → 404, poisoned until the next metadata refresh, never blind-retried (metadata URL takes no `f=` param; `?f=application/json` is a 400). Fetching is one 2-slot queue, displayed frame first then outward; **cursor moves never abort in-flight fetches** (aborting per playback tick starves a cold S3 pvol site, ~18 s first render) — only line/site changes abort. Last good section stays on screen while the current frame loads (StickyImageWMS philosophy) |
| `src/probe.js` | dBZ point-probe chart via the EDR API; exports shared EDR helpers used by crosshair |
| `src/crosshair.js` | Center reticle readout; reads radar pixels via `getData()` + dBZ via probe helpers. Its per-frame value comes from `src/edr/peaks.js`, the same helper the probe chart uses — the two must never disagree about a point, so never re-implement that aggregation here |
...
| `src/ui/serverStatus.js` | Backend-outage banner: says so when a server feeding a **currently visible** layer stops answering, so a frozen map reads as an outage rather than "nothing is happening". Issues no requests — rides radar.js's existing GetCapabilities poll, which already routes 4xx/5xx into its `.catch` via the `!response.ok` throw. Two consecutive failures before speaking (anti-flap), except a host that has never answered this session, where one is enough (nothing on screen to misread). Which categories a down host affects is answered from layer **source URLs**, not layerInfo, so it is right before capabilities ever succeed — the boot-into-outage case. `navigator.onLine === false` switches the wording to the connection and hides the retry; coming back online triggers one immediate retry instead of waiting out the ≤5 min backoff. Wall-clock — no FramePool/`setTime` coupling |
| `src/particles/windParticles.js` | Two exclusive POI rows, one particle layer: "Tuuli 10 m (ECMWF)" (model 10 m wind) and "Sateen liike (tutka)" (the nowcast engine's precipitation-motion field — never call it wind). Both arrive from the MeteoCore EDR area query as a CoverageJSON `Grid`; `?wind=ecmwf|gfs` picks the model, `?wind=radar` forces the radar source (diagnostics; labels read from the source tables). One OVERLAY CANVAS per pane (`canvas.ol-layer.wind-canvas` between `.ol-layers` and the overlay containers, so share.js composites it; attached in `initPaneTraffic`; not an OL layer, `pane.windLayer` is just the visibility handle) repainted from ONE shared WebGL2 context for every pane (the interpolator already wants up to 8) by the controller's own RAF loop, which reads the extent off the map — OL is NOT re-rendered per frame (doing so through an ImageCanvas source made the radar layer's 16 Mpx blit run at 60 Hz on 4K, the frame rate collapsed and every tail smeared). Runs while a row is on, playing or paused. **Model fetch:** the buffered 0.5°-quantized view box with a 4 000 deg² budget — the budget must hold the VIEW, a smaller box leaves edges without particles and a small pan does not move the quantized box (1 200 did that from z4 out) — at the advertised step nearest the window's NEWEST frame (verbatim from metadata, refreshed every 30 min). **Radar fetch:** always the whole domain bbox (19 kB gz; a box outside it is a 400) with `datetime` = the animation window AS AN INTERVAL — the server returns the newest generation anchored inside it and reads an instant as the empty interval t/t (404), so the URL slides with the frame pool's cadence and needs no metadata. Single abortable slot, 4xx poisoned, 5xx retried in 15 s, last good field stays up. `setSource` before `setEnabled` (applyPoiVisibility). radar.js supplies `radarCoverage` (the FMI sites' discs in EPSG:3857) so the radar field is masked to what the composite sees |
| `src/particles/particleGl.js` | The renderer: positions as 16-bit fixed point in RGBA8, field as RGBA8 over a symmetric ±range (A = valid, B = quality: the radar's `motion_quality` — filled blocks draw at a third alpha, so particles concentrate in precipitation; a model field is 255 everywhere), premultiplied ping-pong trail textures with a floored per-frame fade, `gl_VertexID` point draw. No float render targets, no half-float filtering — nothing the interpolator has to probe for. Screen-space speed (no cos(lat) factor — Mercator is conformal, so direction is exact and a 10 m/s wind flows at one pace everywhere); a no-data sample kills the particle, which is what keeps a masked region empty instead of smeared |
| `src/particles/windField.js` | Pure (node-tested by `scripts/test-wind-field.mjs`): Grid CoverageJSON → rows normalised to ascending latitude / columns to ascending longitude whatever axis order or direction the server used (a flipped row order draws a plausible wind that blows the wrong way — silently), `{start,stop,num}` axes, length-1 extra axes, clipped edge cells (the radar grid's first row is 0.118° against 0.148° — lattice anchored on the interior, interior irregularity still refused), null = no data; byte encoding; nearest-step pick (ties → later, clamped to the run); deterministic field URL via `polygonWkt` shared with the area-query helpers |
| `src/analytics.js` | `track()` wrapper for Umami (no-ops if umami is absent) |
| `src/edr/seriesFetch.js` | Request/window bookkeeping shared by the probe chart and the crosshair: `frameWindow`/`sameWindow` (refetch vs repick), `sameTarget` (several WMS layers resolve to one EDR query), and `createFetchSlot` — a single-slot abortable request whose `run` resolves `undefined` when superseded, so a stale response can never repaint fresh UI. Callers keep their own `series` and state machine |
| `src/edr/peaks.js` | Peak-per-frame aggregation shared by the probe chart and the crosshair readout (pure, node-harness testable): `frameIndexAt` nearest-cell rounding + `peaksByFrame` (drop below the parameter's floor, then keep the largest-**magnitude** sample per cell — signed moments need magnitude, not max). Returns `number \| null` per frame, where **0 is a real reading** — test `== null`, never truthiness |
...
| `src/edr/trajectoryQuery.js` | Pure EDR trajectory helpers (areaQuery.js style, node-harness testable): deterministic URLs (3-decimal endpoint quantization, stable param order), temporal-metadata parsing + frame-time snapping (±150 s), `Section` CoverageJSON parsing (`composite` [t,lon,lat] axis ~500 m spacing × `z` heights in m above antenna; null = no echo, out-of-coverage is a 200 all-null response, not an error), `singleVariantOf` pvol→single regex (Estonian `ee-radar-volume-*` deliberately falls through) |
...
Active servers (see `src/config.js` for the full registry): `meteocore.app.meteo.fi/wms` (Finnish + European radar composites — the primary radar source), `view.eumetsat.int` (satellite RGB products, MTG lightning, RDT). The EDR API at `meteocore.app.meteo.fi/edr` (CoverageJSON) feeds the probe/crosshair (`position` queries) and the observation + FMI lightning vector layers (`area` queries on the `fmi-obs` / `fmi-lightning` collections; metadata for these products is seeded statically via `edrLayerInfo` in config.js because they have no GetCapabilities). `wms-obs.app.meteo.fi` is **retired — never reintroduce references to it** (offline 2026-07-12; its products are EDR now).

**The registry holds only servers the app actually talks to.** On 2026-08-17 every dormant entry was deleted rather than left parked: `wms.meteo.fi` and `openwms.fmi.fi` (products superseded by meteocore), `geo.weather.gc.ca` (Environment Canada GeoMet), `geoservices.knmi.nl`, `vietnam.smartmet.fi`, `mesonet.agron.iastate.edu`, and the legacy `eumetview.eumetsat.int` endpoints. Adding a server back means adding its host to the index.html CSP (`connect-src`, plus `img-src` if it serves GetMap) — the CSP was tightened to match, so a config-only revival passes lint and build while the browser blocks every request.

Two per-server config mechanisms died with their last user and are **not** in the code any more: `narrowByLayer` (GeoMet's `&layer=` GetCapabilities narrowing) and `namespace` (GeoServer workspace prefixing on layer names, plus its segment of the endpoint dedup key). Re-add them from git history if a server ever needs them again. `disabled: true` is still honored — it just has no users.
...
- localStorage keys: `metPosition`, `metZoom`, `VISIBLE`, `ACTIVE_LAYERS`, `ACTIVE_STYLES` (per-category WMS style memory — applied by `updateLayer` on every product switch and by `restoreActiveStyle` once GetCapabilities lands; a style the current product doesn't advertise is kept, not evicted), `interpMode`, `IS_DARK`, `IS_TRACKING`, `IS_FOLLOWING`, `LP_HINT_SEEN`, `POI_STATE`, `TOOL_STATE`, `timeIsUtc`, `ownLocationSource` (`'gps'`/`'ais'`, `'nmea'` reserved; self-heals to `gps` when stored `ais` has no usable MMSI), `ownMmsi` (9-digit string, survives source flips; owned by `src/ownLocation.js`) (+ legacy `metLatitude`/`metLongitude` writes). JSON values go through `safeParseJSON` in radar.js.

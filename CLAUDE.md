...
Active servers (see `src/config.js` for the full registry): `meteocore.app.meteo.fi/wms` (Finnish + European radar composites — the primary radar source), `view.eumetsat.int` (satellite RGB products, MTG lightning, RDT). The EDR API at `meteocore.app.meteo.fi/edr` (CoverageJSON) feeds the probe/crosshair (`position` queries) and the observation + FMI lightning vector layers (`area` queries on the `fmi-obs` / `fmi-lightning` collections; metadata for these products is seeded statically via `edrLayerInfo` in config.js because they have no GetCapabilities). Three GeoServers are **retired — never reintroduce references to them**: `wms-obs.app.meteo.fi` (offline 2026-07-12), and `wms.meteo.fi` + `openwms.fmi.fi` (both removed 2026-08-17; every product they served now comes from meteocore, and `*.fmi.fi` is no longer in the index.html CSP). Environment Canada's GeoMet (`geo.weather.gc.ca`) was removed the same day — it was the only entry that ever set `narrowByLayer`, so that GetCapabilities `&layer=` narrowing went with it. Entries still marked `disabled: true` (KNMI, VNMHA, NOAA/mesonet, the legacy eumetview endpoints) are inactive — don't document or build on them, and note their hosts are no longer in the CSP either, so re-enabling one means adding it back to `connect-src`.

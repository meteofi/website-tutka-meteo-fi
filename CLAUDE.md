...
| `src/ais/ownShipStyle.js` | Own-position symbology: the shared blue GPS dot (`gpsPositionStyle`) + the ECDIS S-52 own-ship style function (double circle, true-heading line with COG fallback, dashed 6-min COG/SOG vector, name/kn label) driven by the feature's `aisState` property; AIS sentinel filtering (heading 511 / cog 360 / sog 102.3) happens in ownLocation.js |
| `src/ui/ownLocationMenu.js` | "Oma sijainti" overflow-menu section: GPS/AIS-alus source chips + 9-digit MMSI input; pending-selection flow keeps GPS effective until a valid MMSI commits |

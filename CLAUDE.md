...
| `src/placeNames.js` | Place-name labels (replaced the ArcGIS reference tile layers): one shared VectorSource over the bundled `src/data/placenames-fi.geojson` snapshot + per-pane declutter VectorLayer; zoom-banded visibility via MML `scaleRelevance` (500k/1M/2M/8M), light/dark style functions swapped from `setMapLayer`. Regenerate the data with `MML_API_KEY=<key> node scripts/fetch-placenames.mjs` (key needed only there, never at runtime). Wall-clock static — no FramePool/`setTime` coupling |

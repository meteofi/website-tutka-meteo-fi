...
| `src/particles/windParticles.js` | "Tuuli" POI layer: Windy-style flowing particles over a lon/lat grid of east/north m/s. Model 10 m wind today (MeteoCore EDR area query → CoverageJSON `Grid`; default source GFS until the `ecmwf-ifs` area query stops answering 502 — `?wind=ecmwf` / `?wind=gfs` override; the menu label reads from the source table); radar precipitation motion later through the same wire shape (#256), which must be labelled "sateen liike", never wind. One per-pane `ImageLayer` over an `ImageCanvas` (ratio 1) repainted from ONE shared WebGL2 context for every pane (the interpolator already wants up to 8); its own RAF loop marks visible panes' sources changed, so the simulation steps inside the canvasFunction while the layer is on, playing or paused. One field per (0.5°-quantized view box, model step nearest the window's NEWEST frame) — verbatim step from collection metadata, single abortable slot, 4xx poisoned, 5xx retried in 60 s, last good field stays up. `setEnabled` builds and tears down the GL context; `setCursor` only picks the step |
| `src/particles/particleGl.js` | The renderer: positions as 16-bit fixed point in RGBA8, field as RGBA8 over a symmetric ±range (A = valid, B reserved for the radar quality mask), premultiplied ping-pong trail textures with a floored per-frame fade, `gl_VertexID` point draw. No float render targets, no half-float filtering — nothing the interpolator has to probe for. Screen-space speed (no cos(lat) factor — Mercator is conformal, so direction is exact and a 10 m/s wind flows at one pace everywhere); a no-data sample kills the particle, which is what keeps a masked region empty instead of smeared |

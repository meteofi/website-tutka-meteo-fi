...
export default function createPane(targetEl, sharedView, deps) {
  ...
  // Railway main tracks — a bundled snapshot of Väylä's rail network,
  // regenerated by scripts/fetch-rail-tracks.mjs. Drawn before the stations so
  // the markers sit on top of their own lines. No labels, so no declutter
  // group: the geometry always draws.
  const railwayTrackLayer = new VectorLayer({
    source: new Vector({
      format: new GeoJSON(),
      url: railwayTracksUrl,
      attributions: 'Rataverkko © Väylävirasto',
    }),
    visible: false,
    style: railwayTrackStyle,
  });

  // Railway stations (Rautatieasemat) — a bundled snapshot of the passenger
  // stations from Digitraffic's rail metadata, regenerated by
  // scripts/fetch-rail-stations.mjs. Own declutter group so station labels
  // thin against each other rather than erasing place names.
  const railwayLayer = new VectorLayer({
    source: new Vector({
      format: new GeoJSON(),
      url: railwayStationsUrl,
      attributions: 'Rautatieasemat © Fintraffic (CC BY 4.0)',
    }),
    visible: false,

...
Yellow / orange / red areas use restrained translucent fills, contrasting boundaries and `!` / `!!` / `!!!` badges. Finnish severity names supplement the colors. Future warnings have dashed boundaries. Radar echoes, lightning and observation symbols remain visible. Regional warnings do not describe the exact footprint of a thunderstorm.

A floating summary reports warning areas intersecting the visible map. Tap the summary or a warning area to open a native modal bottom sheet. It has 44 px controls, safe-area padding, keyboard focus containment, Escape/close dismissal, and independent scrolling. Selected overlapping warnings appear first with their details expanded. Existing storm cells and marker cards retain tap priority. The list otherwise orders active warnings before upcoming warnings, then by severity and start time.

Cards show severity and hazard type in the title, region, active/upcoming state, local validity dates and times with explicit time zones, original warning text and instructions, issuer and publication time. Original text carries its source language and is inserted as text, not HTML. Source links accept only HTTP(S). “Näytä alue kartalla” fits the region in the current pane; the shared view moves all panes together. Users can include areas outside the viewport in the list without moving the map.

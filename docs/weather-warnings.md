...
Card titles follow the guide’s color + hazard format, e.g. “Keltainen tuulivaroitus”, “Oranssi ukkosvaroitus” and “Punainen sadevaroitus”. Cards show region, active/upcoming state, local validity dates and times with explicit time zones, original warning text and instructions, issuer and publication time. Original text carries its source language and is inserted as text, not HTML. Source links accept only HTTP(S). “Näytä alue kartalla” fits the region in the current pane; the shared view moves all panes together. Users can include areas outside the viewport in the list without moving the map.

...
Issuer, publication time and the source link follow the body. Original text carries its source language in the HTML `lang` attribute for screen readers, without a visible language-code label, and is inserted as text, not HTML; Finnish section headings have their own language tag. Description/instruction pipe delimiters retain their bullet formatting. Source links accept only HTTP(S). “Näytä alue kartalla” fits the region in the current pane; the shared view moves all panes together. Users can include areas outside the viewport in the list without moving the map.
...
- Filter on the server by the selected `awareness_type`, `status=Actual` and `scope=Public`. The [Features OpenAPI definition](https://meteocore.app.meteo.fi/features/api) specifies exact, case-sensitive equality; repeated property names are ANDed, not ORed. Request both observed spellings separately: `1; wind` / `1; Wind`, `3; thunderstorm` / `3; Thunderstorm`, and `10; rain` / `10; Rain`. Numeric prefixes alone do not match. Any additional full-value spellings would need adding here, or a server-side normalized-code filter.
- Include one `msgType=Cancel` query with the same status/scope filters, because cancellations may omit awareness type and geometry. Merge all selected-type and cancellation results before applying CAP references and local validation. Do not filter by severity or language on the server: updates to green/all-clear warnings and all supplied languages must remain available. One enabled type normally needs three small queries; all three need seven. This trades a few small requests for downloading fewer unrelated features and their geometry.
...
- One in-memory snapshot, spatial index and refresh batch serve every pane. Fetch only while enabled; refresh every five minutes while visible, retry failures after one minute, catch up when the tab returns, and retry on reconnection. Each refresh fetches only the enabled types. Requests time out after 45 seconds and abort on disable. No warning requests are made on pan, zoom, animation, current/24-hour switching or pane creation.
- Removing a type reuses the broader cached snapshot. Enabling a type outside the cached coverage fetches the new selection, replacing any narrower in-flight request. While coverage is incomplete, the sheet reports loading/unavailable rather than a successful empty result. A fresh cached selection can be reused after disabling/re-enabling warnings. Superseded request results cannot replace the active selection.
...
The observed feed on 2026-09-13 was about 105 kB uncompressed for 26 features, so a shared GeoJSON snapshot is a reasonable starting point. Actual cost grows with warning count and polygon complexity; property filtering now reduces transfers to the enabled warning types plus cancellation records.
...
`npm test` includes CAP contract checks for awareness codes, levels, status, all-clear/cancellation, validity boundaries, Polygon/MultiPolygon acceptance, deduplication, safe source links, selected-type/case-variant queries, filtered pagination, repeated pages, partial failures and aborts. Controller tests cover selection changes during requests, incomplete-coverage reporting, late responses, shared pane sources, cache reuse, enabled-type polling and disable cleanup. Lint and production build are required as usual.
